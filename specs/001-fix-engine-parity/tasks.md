...
- [X] T085 [US12] Author the **server** AT scenarios as independent fixtures in `crates/truefix-at/src/scenarios.rs` (no copied source). Class-complete across FIX.4.2/4.4: logon (valid/seq-too-high/HeartBtInt-adoption/ResetOnLogon/789/369), sequencing (too-high/too-low/PossDup/missing-seqnum/out-of-order queue+drain), SequenceReset/ResendRequest (Reset, GapFill fwd+back, open/bounded/begin-0/nothing/dedup), admin consumption (Heartbeat/Reject/unsolicited-Logout), validity/reject (14a–14f + 2r on both versions + valid-accept), application (NewOrderSingle→ExecutionReport, outbound sequencing, app-resend PossDup, mixed GapFill+PossDup), timer-driven (idle Heartbeat, TestRequest-on-silence, acceptor-initiated Logout). 48 scenarios / 76 runs, gated by `conformance.rs`. NOTE: representative scenario(s) per behavior class rather than a literal 73-count; remaining Appendix B entries are value-permutations of covered paths. The 789/369 suites surfaced & fixed a real conformance bug.
- [X] T086 [P] [US12] Special-category suites authored in `crates/truefix-at/src/scenarios.rs`: nextExpectedMsgSeqNum, lastMsgSeqNumProcessed, resendRequestChunkSize, validateChecksum/rejectGarbledMessages (default-drop), timestamps (CheckLatency). `resynch` covered at the transport level (`reconnect.rs`, `restart_continuity.rs`) (Appendix B; CHK027)
