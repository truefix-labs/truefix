...
workspace crate. `truefix-okx-client` 0.1.7 is a focused wire-format hotfix which keeps every
order Decimal encoded as the string required by OKX even when downstream Cargo feature unification
enables `rust_decimal/serde-float`; the remaining published workspace crates stay on `0.1.6`.
