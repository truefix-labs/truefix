...
/// T068 (US9, feature 005) — `--format qfj` converts a QuickFIX-schema XML dictionary (as
/// opposed to the default `orchestra` format), gated on `--version` being supplied. See
/// `dictionary_coverage.rs` for the full per-version regeneration-matches-shipped-file coverage.
#[test]
fn generate_dict_format_qfj_converts_a_quickfix_schema_xml_source() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
        ...
        "qfj",
        "--source",
        "../../thrdpty/quickfix/spec/FIX40.xml",
        "--version",
        "FIX.4.0",
        "--out",
        out_path.to_str().unwrap(),
    ...
    let generated = std::fs::read_to_string(&out_path).unwrap();
    assert!(generated.starts_with("# Generated by truefix-dict's QuickFIX-XML conversion tool"));
    assert!(generated.contains("version FIX.4.0\n"));
}

#[test]
fn generate_dict_format_qfj_without_version_is_a_clean_error() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
