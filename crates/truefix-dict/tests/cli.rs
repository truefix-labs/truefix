...
/// T068 (US9, feature 005) — `--format fix-repository` converts the FIX Trading Community's
/// official "Unified Repository" XML (as opposed to the default `orchestra` format), gated on
/// `--version` being supplied. See `dict-src/fix-repository/PROVENANCE.md` for why this reads a
/// vendored, Apache-2.0-licensed local source rather than a third-party reference engine's own
/// bundled data files. See `dictionary_coverage.rs` for broader per-version coverage.
#[test]
fn generate_dict_format_fix_repository_converts_a_repository_xml_source() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
        ...
        "fix-repository",
        "--source",
        "dict-src/fix-repository/FIX.4.0",
        "--version",
        "FIX.4.0",
        "--out",
        out_path.to_str().unwrap(),
    ...
    let generated = std::fs::read_to_string(&out_path).unwrap();
    assert!(generated.starts_with("# Generated by truefix-dict's FIX Repository conversion tool"));
    assert!(generated.contains("version FIX.4.0\n"));
}

#[test]
fn generate_dict_format_fix_repository_without_version_is_a_clean_error() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
...
/// T028 (US3, feature 011, FR-015): `--format vendor-xml` converts a vendor-published dictionary
/// XML file (Binance's published shape) the same way the other two formats do.
#[test]
fn generate_dict_format_vendor_xml_converts_a_vendor_xml_source() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
        ...
        "vendor-xml",
        "--source",
        "tests/fixtures/vendor_dict_sample.xml",
        "--out",
        out_path.to_str().unwrap(),
    ]);
    assert!(
    ...
    let generated = std::fs::read_to_string(&out_path).unwrap();
    assert!(generated.starts_with("# Generated by truefix-dict's vendor-XML conversion tool"));
    assert!(generated.contains("version FIX.4.4\n"));
    assert!(generated.contains("message XCN TestCustomConfirmation"));
}

#[test]
fn generate_dict_unknown_format_is_a_clean_error() {
    let dir = scratch_dir();
    let out_path = dir.join("generated.fixdict");
    let out = run(&[
        "generate-dict",
