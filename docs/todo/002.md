...
> **Historical audit snapshot.** Findings and source locations refer to the 2026-07-02 revision;
...
> `docs/todo/001.md`'s "不适合 Rust" section, not repeated here), and TrueFix's own unique
...
> repeated; see `docs/todo/001.md`. One caveat found during this pass: GAP-05's "`.cfg`-only
...
requires_orig_sending_time` dictionary toggle (see `docs/todo/001.md` TODO-09) — that toggle
...
- `SocketAcceptProtocol` / `SocketConnectProtocol` (`keys.rs:132,149`) should almost certainly be
  ...
  — an in-JVM-process transport with no meaningful Rust equivalent. `docs/todo/001.md`
