...
| **Phase 2** | 二进制协议扩展 | FAST/SBE 二进制编解码、STEP 协议、低延迟市场数据通道 |
| **Phase 3** | 多券商 API 网关 | IB TWS / OpenD / Binance 适配器、独立 Client SDK、统一 Instruments、IB→FIX 协议桥接 |
...
### 3.9 独立 Client SDK：`truefix-binance-client` / `truefix-ib-client` / `truefix-futu-client`
...
| 3r | `truefix-ib-client` | 独立可用；IB Gateway 无头模式下单/行情验证通过 Paper Trading |
...
- **协议转换层**：`NewOrderSingle(D)` → `truefix-ib-client::place_order()`；IB 的 `execDetails`/`orderStatus`
